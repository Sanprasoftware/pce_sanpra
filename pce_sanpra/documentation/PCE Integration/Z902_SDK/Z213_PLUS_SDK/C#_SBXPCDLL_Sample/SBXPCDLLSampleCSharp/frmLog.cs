...
namespace SBXPCDLLSampleCSharp
{
	public partial class frmLog : Form
    {
		...
		private void prepareGLog()
		{
            ...
            chkShowPhoto.Visible = true;
            grdGlog.Visible = true;
            grdGlog.Columns["colBodyTemperature"].Visible = chkBodyTemperature.Checked;
            prevGlogIndex = -1;
        }
        ...
        private void cmdGlogData_Click(object sender, EventArgs e)
        {
            ...
            if (chkBodyTemperature.Checked)
                vRet = sbxpc.SBXPCDLL.ReadGeneralLogData_Ext(Program.gMachineNumber, chkReadMark.Checked ? (byte)1 : (byte)0);
            else
            ...
            if (vRet)
            {
				...
				while (true)
				{
                    ...
                    if (chkBodyTemperature.Checked)
                        vRet = sbxpc.SBXPCDLL.GetGeneralLogData_Ext(Program.gMachineNumber,
													out gi.tmno,
													out gi.seno,
													out gi.smno,
													out gi.vmode,
        ...
        private void cmdAllGLogData_Click(object sender, EventArgs e)
        {
            ...
            if (chkBodyTemperature.Checked)
                vRet = sbxpc.SBXPCDLL.ReadAllGLogData_Ext(Program.gMachineNumber);
            else
            ...
            if (vRet)
            {
				...
				while (true)
				{
					GLogInfo gi = new GLogInfo();
                    if (chkBodyTemperature.Checked)
                        vRet = sbxpc.SBXPCDLL.GetAllGLogData_Ext(Program.gMachineNumber,
                                                out gi.tmno,
                                                out gi.seno,
                                                out gi.smno,
                                                out gi.vmode,
		...
		private void cmdReadGLogWithPos_Click(object sender, EventArgs e)
		{
            ...
            bool ret;
            if (chkBodyTemperature.Checked)
                ret = sbxpc.SBXPCDLL.ReadGLogWithPos_Ext(Program.gMachineNumber, start, end);
            else
                ret = sbxpc.SBXPCDLL.ReadGLogWithPos(Program.gMachineNumber, start, end);
        ...
        private void grdGlog_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (!chkShowPhoto.Checked)
                return;

            if (prevGlogIndex == e.RowIndex)
                return;

            prevGlogIndex = e.RowIndex;
            var gi = glogs_[prevGlogIndex];
            if (gi.photo == "No Photo")
            {
                lblMessage.Text = "No Photo Id";
                return;
            ...
            bool vRet = sbxpc.SBXPCDLL.EnableDevice(Program.gMachineNumber, 0); // 0 : false
            if (!vRet)
            {
                lblMessage.Text = util.gstrNoDevice;
                return;
            ...
            string strXML = null;
            sbxpc.SBXPCDLL.XML_AddString(ref strXML, "REQUEST", "GetGLogPhotoData");
            sbxpc.SBXPCDLL.XML_AddString(ref strXML, "MSGTYPE", "request");
            sbxpc.SBXPCDLL.XML_AddLong(ref strXML, "MachineID", Program.gMachineNumber);
            sbxpc.SBXPCDLL.XML_AddInt(ref strXML, "PhotoPos", photoPos);
            ...
            if (sbxpc.SBXPCDLL.GeneralOperationXML(Program.gMachineNumber, ref strXML))
            {
                ...
                int nPhotoSize = util.gCompressPhotoSize_8K;
                try
                {
                    nPhotoSize = sbxpc.SBXPCDLL.XML_ParseInt(ref strXML, "PhotoSize");
                }
                catch
                {
                }

                Byte[] photoData = new Byte[nPhotoSize];
                GCHandle gh = GCHandle.Alloc(photoData, GCHandleType.Pinned);
                IntPtr AddrOfPhotoData = gh.AddrOfPinnedObject();

                vRet = sbxpc.SBXPCDLL.XML_ParseBinaryLong(ref strXML, "PhotoData", AddrOfPhotoData, nPhotoSize);
		...
		public string manipulation
		{
			get
			{
				switch (mnpl)
				{
                    ...
                    case 15:
                        return Convert.ToString(mnpl) + "--" + "Reset All Settings";
                    case 16:
                        return Convert.ToString(mnpl) + "--" + "Delete All Log";
                    case 19:
                        return Convert.ToString(mnpl) + "--" + "Modify UserTimezone";
                    case 20:
                        return Convert.ToString(mnpl) + "--" + "Turn On";
                    case 24:
                        return Convert.ToString(mnpl) + "--" + "DoorOpenTimeout Alarm";
                    case 25:
                        return Convert.ToString(mnpl) + "--" + "Illegal DoorOpen Alarm";
                    case 26:
                        return Convert.ToString(mnpl) + "--" + "Duress Alarm";
                    case 28:
                        return Convert.ToString(mnpl) + "--" + "Enroll Photo";
                    case 29:
                        return Convert.ToString(mnpl) + "--" + "Delete Photo";
                    case 31:
                        return Convert.ToString(mnpl) + "--" + "Tamper Alarm";
                    case 32:
                        return Convert.ToString(mnpl) + "--" + "Enter Menu";
                    case 33:
                        return Convert.ToString(mnpl) + "--" + "Delete User";

                    case 34:
                        return Convert.ToString(mnpl) + "--" + "AbnormalBodyTemperature Alarm";
	...
	class GLogInfo
	{
        ...
        public string body_temperature
        {
            get
            {
                const int BodyTemperature100_Glog_Delta0 = 3000;

                int body_temperature_10_delta = (dwGlogExt & 0xFF); // (body_temperature_10_delta: 8, reserved: 24)

                if (body_temperature_10_delta > 0)
                {
                    int body_temperature_100 = body_temperature_10_delta;
                    body_temperature_100 = body_temperature_100 * 10 + BodyTemperature100_Glog_Delta0;
                    return (body_temperature_100 / 100).ToString() + "." + ((body_temperature_100 % 100) / 10).ToString() + "'C";
                }
                else
                    return "";
            }
